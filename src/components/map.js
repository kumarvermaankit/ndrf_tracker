...
function Map() {
    const url = "https://ndrfassemble.herokuapp.com"
    ...
    useEffect(() => {
        async function getUserPins() {
            try {
                const res = await axios.get(`${url}/getsinglefeedinfo/1`)
                console.log(res)
                    ...
                    return (
                        e.map((p) => {
                            return (
                                ...
                                <div key={p._id}>
                                    <Marker latitude={Number(p.disisterlocationlatitude)} longitude={Number(p.disisterlocationlongitude)} offsetLeft={-(viewport.zoom * 3.5)} offsetTop={-(viewport.zoom * 7)}>
                                        <LocationOnIcon style={{ fontSize: viewport.zoom * 7, color: "purple", cursor: "pointer" }}
                                            onClick={() => markerClick(p._id, Number(p.disisterlocationlatitude), Number(p.disisterlocationlongitude))}
                                    ...
                                    {currentplace === p._id &&
                                        ...
                                        <Popup
                                            latitude={Number(p.disisterlocationlatitude)}
                                            longitude={Number(p.disisterlocationlongitude)}
                                            closeButton={true}
                                            closeOnClick={false}
                                            ...
                                            <div className="card">
                                                ...
                                                <span className="username">Timing <b>{p.timing}</b></span>
                                                <span className="date">{format(p.created_at)}</span>
                                            </div>
                                        </Popup>
                                    }
                        ...
                        <div key={p._id}>
                            ...
                            {currentplace === p._id &&
                                <Popup
                                    ...
                                    <div className="card" style={{ display: "flex", flexDirection: "column" }}>
                                        ...
                                        {currentuser.username !== p.username && <button onClick={(event) => StartChat(event, "hdcjj", p.id)}>Chat</button>}
                                        {/* <label>Information</label> */}
                            ...
                            {msgbox[p.id] ?
                                <Popup
                                    ...
                                    <Messagebox id={p._id} rN={params.id} CloseChat={onClose} currentuser={currentuser.username} username={p.username} messages={messages} messagesetter={MessageSetter} socket={socket} />
